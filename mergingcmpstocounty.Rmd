...
```{r Step 4 Convert numbers to county names, include=TRUE}
...
```{r Step 5 Merge County Lookup Table to CMPS Data, include=TRUE}
...
# Also need a state lookup to get state names
state_fips_lookup <- countylookuptable %>%
  distinct(`State FIPS Code`, `State Name`) %>%
  filter(!is.na(`State FIPS Code`) & !is.na(`State Name`))

# Add state names to your data
cmps2024 <- cmps2024 %>%
  mutate(s21_numeric = as.numeric(s21)) %>%
  left_join(state_fips_lookup, by = c("s21_numeric" = "State FIPS Code"))

# Now match county names to get FIPS codes  
cmps2024 <- cmps2024 %>%
  left_join(county_fips_lookup, 
            by = c("State Name" = "State Name", "county_name" = "County Name")) %>%
