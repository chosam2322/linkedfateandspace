...
cmps2024 <- read_dta(here("cmps2024_primaryadult_091025.dta"))
civicorgdata <- read_csv(here("mma_sc_demo.csv"))

```

```{r Step 1: Checking Data for Missingness on County, include=TRUE}
# Check if all county variables (akc to wyc) are NA for each respondent
cmps2024 <- cmps2024 %>%
  mutate(no_county_response = if_all(akc:wyc, is.na))

# Count how many respondents have no county response
sum(cmps2024$no_county_response, na.rm = TRUE)

```

```{r Step 2: Get County Code Numbers for Respondents, include=TRUE}
# Function to get county code based on state
get_county_code <- function(data) {
  data %>%
    rowwise() %>%
    mutate(
...
cmps2024 <- get_county_code(cmps2024)
```

```{r Step 2: Create lookup tables for county names, include=TRUE}
...
# Arizona (15 counties)  
 "3" = c(
    "1" = "Apache", "2" = "Cochise", "3" = "Coconino","4" = "Gila", "5" = "Graham", "6" = "Greenlee", "7" = "La Paz", "8" = "Maricopa", "9" = "Mohave", "10" = "Navajo", "11" = "Pima", "12" = "Pinal",  "13" = "Santa Cruz",  "14" = "Yavapai", "15" = "Yuma"
),
...
# Arkansas
 ...
 # Nebraska (93 counties)
  ...
  "32" = c(
    ...
    "5" = "Colfax", "6" = "Curry", "7" = "De Baca", "8" = "Doña Ana", 
...
# Updated convert_county_codes function
convert_county_codes <- function(data) {
  
  data_with_counties <- data %>%
    mutate(
      # Convert s21 numeric to state name using lookup
      state_name = state_lookup[as.character(s21)],
      # Convert all county code variables to character
      akc = as.character(akc), alc = as.character(alc), azc = as.character(azc),
      arc = as.character(arc), cac = as.character(cac), coc = as.character(coc),
      ctc = as.character(ctc), dec = as.character(dec), flc = as.character(flc),
      gac = as.character(gac), hic = as.character(hic), iac = as.character(iac),
      idc = as.character(idc), ilc = as.character(ilc), inc = as.character(inc),
      ksc = as.character(ksc), kyc = as.character(kyc), lac = as.character(lac),
      mac = as.character(mac), mec = as.character(mec), mdc = as.character(mdc),
      mic = as.character(mic), mnc = as.character(mnc), moc = as.character(moc),
      msc = as.character(msc), mtc = as.character(mtc), ncc = as.character(ncc),
      ndc = as.character(ndc), nec = as.character(nec), nhc = as.character(nhc),
      nmc = as.character(nmc), nvc = as.character(nvc), njc = as.character(njc),
      nyc = as.character(nyc), ohc = as.character(ohc), okc = as.character(okc),
      orc = as.character(orc), pac = as.character(pac), ric = as.character(ric),
      scc = as.character(scc), sdc = as.character(sdc), tnc = as.character(tnc),
      txc = as.character(txc), utc = as.character(utc), vac = as.character(vac),
      vtc = as.character(vtc), wac = as.character(wac), wic = as.character(wic),
      wvc = as.character(wvc), wyc = as.character(wyc)
    ) %>%
    mutate(
      county_code = case_when(
        state_name == "Alaska" & !is.na(akc) & akc != "NA" ~ akc,
        state_name == "Alabama" & !is.na(alc) & alc != "NA" ~ alc,
        state_name == "Arizona" & !is.na(azc) & azc != "NA" ~ azc,
        state_name == "Arkansas" & !is.na(arc) & arc != "NA" ~ arc,
        state_name == "California" & !is.na(cac) & cac != "NA" ~ cac,
        state_name == "Colorado" & !is.na(coc) & coc != "NA" ~ coc,
        state_name == "Connecticut" & !is.na(ctc) & ctc != "NA" ~ ctc,
        state_name == "Delaware" & !is.na(dec) & dec != "NA" ~ dec,
        state_name == "Florida" & !is.na(flc) & flc != "NA" ~ flc,
        state_name == "Georgia" & !is.na(gac) & gac != "NA" ~ gac,
        state_name == "Hawaii" & !is.na(hic) & hic != "NA" ~ hic,
        state_name == "Iowa" & !is.na(iac) & iac != "NA" ~ iac,
        state_name == "Idaho" & !is.na(idc) & idc != "NA" ~ idc,
        state_name == "Illinois" & !is.na(ilc) & ilc != "NA" ~ ilc,
        state_name == "Indiana" & !is.na(inc) & inc != "NA" ~ inc,
        state_name == "Kansas" & !is.na(ksc) & ksc != "NA" ~ ksc,
        state_name == "Kentucky" & !is.na(kyc) & kyc != "NA" ~ kyc,
        state_name == "Louisiana" & !is.na(lac) & lac != "NA" ~ lac,
        state_name == "Massachusetts" & !is.na(mac) & mac != "NA" ~ mac,
        state_name == "Maine" & !is.na(mec) & mec != "NA" ~ mec,
        state_name == "Maryland" & !is.na(mdc) & mdc != "NA" ~ mdc,
        state_name == "Michigan" & !is.na(mic) & mic != "NA" ~ mic,
        state_name == "Minnesota" & !is.na(mnc) & mnc != "NA" ~ mnc,
        state_name == "Missouri" & !is.na(moc) & moc != "NA" ~ moc,
        state_name == "Mississippi" & !is.na(msc) & msc != "NA" ~ msc,
        state_name == "Montana" & !is.na(mtc) & mtc != "NA" ~ mtc,
        state_name == "North Carolina" & !is.na(ncc) & ncc != "NA" ~ ncc,
        state_name == "North Dakota" & !is.na(ndc) & ndc != "NA" ~ ndc,
        state_name == "Nebraska" & !is.na(nec) & nec != "NA" ~ nec,
        state_name == "New Hampshire" & !is.na(nhc) & nhc != "NA" ~ nhc,
        state_name == "New Mexico" & !is.na(nmc) & nmc != "NA" ~ nmc,
        state_name == "Nevada" & !is.na(nvc) & nvc != "NA" ~ nvc,
        state_name == "New Jersey" & !is.na(njc) & njc != "NA" ~ njc,
        state_name == "New York" & !is.na(nyc) & nyc != "NA" ~ nyc,
        state_name == "Ohio" & !is.na(ohc) & ohc != "NA" ~ ohc,
        state_name == "Oklahoma" & !is.na(okc) & okc != "NA" ~ okc,
        state_name == "Oregon" & !is.na(orc) & orc != "NA" ~ orc,
        state_name == "Pennsylvania" & !is.na(pac) & pac != "NA" ~ pac,
        state_name == "Rhode Island" & !is.na(ric) & ric != "NA" ~ ric,
        state_name == "South Carolina" & !is.na(scc) & scc != "NA" ~ scc,
        state_name == "South Dakota" & !is.na(sdc) & sdc != "NA" ~ sdc,
        state_name == "Tennessee" & !is.na(tnc) & tnc != "NA" ~ tnc,
        state_name == "Texas" & !is.na(txc) & txc != "NA" ~ txc,
        state_name == "Utah" & !is.na(utc) & utc != "NA" ~ utc,
        state_name == "Virginia" & !is.na(vac) & vac != "NA" ~ vac,
        state_name == "Vermont" & !is.na(vtc) & vtc != "NA" ~ vtc,
        state_name == "Washington" & !is.na(wac) & wac != "NA" ~ wac,
        state_name == "Wisconsin" & !is.na(wic) & wic != "NA" ~ wic,
        state_name == "West Virginia" & !is.na(wvc) & wvc != "NA" ~ wvc,
        state_name == "Wyoming" & !is.na(wyc) & wyc != "NA" ~ wyc,
        TRUE ~ NA_character_
      )
    ) %>%
...
# =============================================================================
# FIPS CODE CREATION FUNCTION
# =============================================================================

# Fixed FIPS CODE CREATION FUNCTION using base R approach
create_fips_codes <- function(data_with_counties) {
  ...
  # Get the data and convert to regular data frame
  county_fips_raw <- counties(cb = TRUE)
  county_fips_df <- as.data.frame(county_fips_raw)
  
  # Select columns using base R and create FIPS codes
  county_fips <- county_fips_df[, c("STATEFP", "COUNTYFP", "STATE_NAME", "NAMELSAD")]
  
  county_fips <- county_fips %>%
    mutate(
      FIPS = paste0(STATEFP, COUNTYFP),  # Create 5-digit FIPS code
      # Clean county names for better matching
      COUNTY_CLEAN = str_remove_all(NAMELSAD, " County| Parish| Borough| Census Area"),
  ...
  data_with_fips <- data_with_counties %>%
    ...
    left_join(
      county_fips, 
      by = c("county_clean" = "COUNTY_CLEAN", "state_name" = "STATE_NAME")
    ) %>%
...
# Function to check for missing or invalid county codes
check_county_coverage <- function(survey_data) {
  
  coverage_report <- survey_data %>%
    count(s21, name = "total_records") %>%
    mutate(state_name = as.character(s21)) %>%
    select(-s21)
...
# Function to identify unmatched counties
identify_unmatched_counties <- function(processed_data) {
  
  unmatched <- processed_data %>%
    filter(match_quality == "no_match" | is.na(FIPS)) %>%
    count(state_name, county_name, county_code, sort = TRUE)
  
  if (nrow(unmatched) > 0) {
    message("Unmatched counties requiring manual review:")
    print(unmatched)
  } else {
    message("All counties successfully matched!")
  }
...
# Test the system
message("County lookup system loaded successfully!")
message("Run: cmps_merged <- process_survey_data(cmps2024)")

```

```{r Run the processing pipeline, include=TRUE}
cmps_merged <- process_survey_data(cmps2024)
```

```{r Update unmatched counties, include=TRUE}
# Create manual FIPS lookup for unmatched counties
manual_fips_lookup <- tribble(
  ~county_clean, ~state_name, ~FIPS,
...
# Function to add manual FIPS codes to unmatched records
add_manual_fips <- function(processed_data) {
  
  # First, update records that didn't match
  updated_data <- processed_data %>%
    # Clean county names for manual matching
    mutate(
      county_clean_manual = str_remove_all(county_name, " County| Parish| Borough| Census Area"),
      county_clean_manual = str_trim(county_clean_manual)
    ) %>%
    # Add manual FIPS codes for unmatched records
    left_join(
      manual_fips_lookup, 
      by = c("county_clean_manual" = "county_clean", "state_name" = "state_name"),
      suffix = c("", "_manual")
    ) %>%
    # Use manual FIPS where original is missing
    mutate(
      FIPS = coalesce(FIPS, FIPS_manual),
      STATEFP = coalesce(STATEFP, str_sub(FIPS_manual, 1, 2)),
      COUNTYFP = coalesce(COUNTYFP, str_sub(FIPS_manual, 3, 5)),
      match_quality = case_when(
        !is.na(FIPS) & is.na(FIPS_manual) ~ match_quality,  # Keep original
        !is.na(FIPS_manual) ~ "manual_match",               # Manual match
        TRUE ~ "no_match"                                    # Still no match
      )
    )
  
  # Remove temporary columns using base R
  cols_to_remove <- c("county_clean_manual", "FIPS_manual")
  updated_data <- updated_data[, !names(updated_data) %in% cols_to_remove]
  
  # Report updated matching statistics
  match_stats <- updated_data %>%
    filter(!is.na(county_name)) %>%
    count(match_quality) %>%
    mutate(percent = round(n / sum(n) * 100, 1))
  
  message("Updated FIPS matching results:")
  print(match_stats)
...
```{r Update dataset with updated FIPS codes, include=TRUE}
# Apply the manual fixes
cmps_merged_fixed <- add_manual_fips(cmps_merged)

# Check remaining unmatched counties
remaining_unmatched <- cmps_merged_fixed %>%
  filter(match_quality == "no_match" & !is.na(county_name)) %>%
  count(state_name, county_name, county_code, sort = TRUE)

message("Remaining unmatched counties:")
print(remaining_unmatched)
```

```{r Merge with Civic Organization Data, include=TRUE}
# Merge the survey data with civic organization data
cmps_merged_final <- cmps_merged_fixed %>%
  left_join(civicorgdata, by = "FIPS")

write.csv(cmps_merged_final, here("cmps_merged_civicorgdata.csv"), row.names = FALSE)
```